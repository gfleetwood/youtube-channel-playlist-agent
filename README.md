...
I used gnurecutils in page of a traditional database. The tasks.rec file contains the list of channels and/or playlists to fetch from, a prompt to use, and the last time the process ran. The Hyperbrowser agent executes the prompts and returns structured JSON that is parsed and sent to me by email using Resend.
